...
def run(config, software, im_fns, check=True):
    ...
    x_dim, y_dim = im_ref_array.shape # this is the part needes changing
    ...
    for i in range(num_of_data):
        if config.registration == 'none':
            ...
            im_file = os.path.join(result_dir, 'L0_Iter0_' + str(i) + '.nii.gz')
    ...
    lr_fn = pyLAR.saveImagesFromDM2D(low_rank, os.path.join(result_dir, 'L' + '_LowRank_'), reference_im_fn)
    sp_fn = pyLAR.saveImagesFromDM2D(sparse, os.path.join(result_dir, 'L' + '_Sparse_'), reference_im_fn)
